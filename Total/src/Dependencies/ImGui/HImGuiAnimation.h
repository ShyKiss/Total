...
namespace HAnimationSystem
{
	...
	*/
	...
	namespace PlayerCallBack
	{
		...
		{
			ImVec4 buff(1, 1, 1, 1);
			buff.x = HAnimationSystem::PlayerCallBack::LinearInterpolation(a.x, b.x, af);
			buff.y = HAnimationSystem::PlayerCallBack::LinearInterpolation(a.y, b.y, af);
			buff.z = HAnimationSystem::PlayerCallBack::LinearInterpolation(a.z, b.z, af);
			return buff;
		}
		inline std::string TotalTextInterpolation(const std::vector<std::string> values, float a, float b, float alpha)
		{
			return values[(int)(a + (b - a) * alpha)];
		}
