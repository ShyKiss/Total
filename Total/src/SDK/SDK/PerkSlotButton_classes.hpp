#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PerkSlotButton

#include "Basic.hpp"

#include "OPP_structs.hpp"
#include "UMG_classes.hpp"


namespace SDK
{

// WidgetBlueprintGeneratedClass PerkSlotButton.PerkSlotButton_C
// 0x01C0 (0x0420 - 0x0260)
class UPerkSlotButton_C final : public UUserWidget
{
public:
	class UTextBlock*                             PerkNameText;                                      // 0x0260(0x0008)(BlueprintVisible, ExportObject, ZeroConstructor, InstancedReference, IsPlainOldData, RepSkip, NoDestructor, PersistentInstance, HasGetValueTypeHash)
	class UPerkUpgradeButton_C*                   PerkUpgradeButton;                                 // 0x0268(0x0008)(BlueprintVisible, ExportObject, ZeroConstructor, InstancedReference, IsPlainOldData, RepSkip, NoDestructor, PersistentInstance, HasGetValueTypeHash)
	int32                                         SlotIndex;                                         // 0x0270(0x0004)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         AvailablePerkSlots;                                // 0x0274(0x0004)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FMenuUpgradeItemInfo                   UpgradeInfo;                                       // 0x0278(0x0190)(Edit, BlueprintVisible)
	bool                                          DisplayPlus;                                       // 0x0408(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn)
	bool                                          DisplayTitle;                                      // 0x0409(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn)
	uint8                                         Pad_40A[0x6];                                      // 0x040A(0x0006)(Fixing Size After Last Property [ Dumper-7 ])
	class ARBPlayerState*                         RBPlayerState;                                     // 0x0410(0x0008)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnTemplate, IsPlainOldData, NoDestructor, ExposeOnSpawn, HasGetValueTypeHash)
	bool                                          ShouldUseMenuLoadout;                              // 0x0418(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn)
	EPerkCategory                                 PerkCategory;                                      // 0x0419(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          bIsLoadout;                                        // 0x041A(0x0001)(Edit, BlueprintVisible, ZeroConstructor, IsPlainOldData, NoDestructor, ExposeOnSpawn)

public:
	void Setup();
	void Init(int32 Slot_0, EPerkCategory PerkCategory_0);
	void Refresh();
	void SetHovered(bool bHovered);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"PerkSlotButton_C">();
	}
	static class UPerkSlotButton_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPerkSlotButton_C>();
	}
};
static_assert(alignof(UPerkSlotButton_C) == 0x000008, "Wrong alignment on UPerkSlotButton_C");
static_assert(sizeof(UPerkSlotButton_C) == 0x000420, "Wrong size on UPerkSlotButton_C");
static_assert(offsetof(UPerkSlotButton_C, PerkNameText) == 0x000260, "Member 'UPerkSlotButton_C::PerkNameText' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, PerkUpgradeButton) == 0x000268, "Member 'UPerkSlotButton_C::PerkUpgradeButton' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, SlotIndex) == 0x000270, "Member 'UPerkSlotButton_C::SlotIndex' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, AvailablePerkSlots) == 0x000274, "Member 'UPerkSlotButton_C::AvailablePerkSlots' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, UpgradeInfo) == 0x000278, "Member 'UPerkSlotButton_C::UpgradeInfo' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, DisplayPlus) == 0x000408, "Member 'UPerkSlotButton_C::DisplayPlus' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, DisplayTitle) == 0x000409, "Member 'UPerkSlotButton_C::DisplayTitle' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, RBPlayerState) == 0x000410, "Member 'UPerkSlotButton_C::RBPlayerState' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, ShouldUseMenuLoadout) == 0x000418, "Member 'UPerkSlotButton_C::ShouldUseMenuLoadout' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, PerkCategory) == 0x000419, "Member 'UPerkSlotButton_C::PerkCategory' has a wrong offset!");
static_assert(offsetof(UPerkSlotButton_C, bIsLoadout) == 0x00041A, "Member 'UPerkSlotButton_C::bIsLoadout' has a wrong offset!");

}

